...
class Tran:
    ...
    def dfs(self,NODE,argu = False):
        ...
        if NODE.getData() == '[function]':
            ...
            self.dfs(NODE._children[0])
            if NODE._children[0]._children!=[]:
                print(NODE._children[0]._children[0].getData(),NODE._children[0]._children[1].getVal())
            if NODE._children[0].getData() == '[None]':
                self.RETURN='None'
            elif NODE._children[0].getVal() != None:
                if self.RETURN!=None:
                    self.RETURN+=NODE._children[0].getVal()
                else:
                    self.RETURN = NODE._children[0].getVal()
            else:
                if self.RETURN != None:
                    self.RETURN+=self.VariableTable[NODE._children[0].getData()]
                else:
                    self.RETURN = self.VariableTable[NODE._children[0].getData()]
