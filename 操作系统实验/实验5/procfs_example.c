...
static ssize_t read_foo_proc(struct file *filp,char __user *buf,size_t count,loff_t *offp ) {
	printk(KERN_INFO"read_foo_proc1: count=%d\n", count);
	...
	printk(KERN_INFO"read_foo_proc2: count=%d  length(tempstr)=%d\n", count, strlen(tempstr));
	//向用户空间写入tempstr
...
static ssize_t write_foo_proc(struct file *filp,const char __user *buf,size_t count,loff_t *offp ){
	int len;
	printk(KERN_INFO"write_foo_proc: count=%d", count);
	if (count>FOOBAR_LEN)
		len=FOOBAR_LEN;
	else
		len=count;
...
static int __init init_procfs_example(void){
	...
	foo_file=proc_create("foo", 0666, example_dir, &foo_proc_fops);
	if (foo_file==NULL){
		rv=-ENOMEM;
		goto no_foo;
	}
	...
	bar_len=strlen(bar_data.name)+strlen(bar_data.value);
	bar_temp=bar_len+4;
	bar_file=proc_create("bar", 0666, example_dir, &bar_proc_fops);
	if (bar_file==NULL){
		rv=-ENOMEM;
		goto no_bar;
	}
